...
define([
    ...
    return {
        ...
        prepareAddress: function(type) {
            ...
            if (address.regionId > 0 && address.region === '') {
                if (window.registry) {
                    $.each(window.registry.get('checkoutProvider')['dictionaries']['region_id'], function(i, region) {
                        if (region.value === address.regionId) {
                            address.region = region.title;
                            return false;
                        }
                    });
                } else {
                    address.region = $('.onestep-shipping-address select[name="region_id"]').find('option:selected').text();
                    $('.onestep-shipping-address input[name="region"]').val(address.region);
                }
            }
            name = {
                "full": fullname
            ...
            result["address"] = {
                "line1": street,
                "city": address.city,
                "state": address.regionCode ? address.regionCode : address.region,
