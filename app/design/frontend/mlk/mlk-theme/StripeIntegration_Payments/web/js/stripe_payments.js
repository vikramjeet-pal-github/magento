...
var stripe =
{
    ...
    initStripeElements: function()
    {
        ...
        cardCvc.addEventListener('blur', function(){
            document.getElementById('card-wrapper').classList.remove('card-focused');
            if (document.querySelector('#step-payment-method .checkout-next-step') != null) {
                document.querySelector('#step-payment-method .checkout-next-step').classList.remove('inactive');
            }
        });
    },
    ...
    /**
     * VONNDA-MODIFICATION:
     * Added call to custom function
     */
    stripeElementsOnChange: function(event)
    {
        stripe.emitCardInputEvent(event);
        ...
        jQuery('#co-payment-form').find('.messages .message-error').remove();
    },
    ...
    getSourceOwner: function()
    {
        ...
        if (stripe.quote)
        {
            ...
            if (stripe.quote.guestEmail)
                ...
                if(stripe.customer 
                    && stripe.customer.customerData
                    && stripe.customer.customerData.email){
                    owner.email = stripe.customer.customerData.email;
                } else if(window.customerEmail) {
                    owner.email = window.customerEmail;
                }
    ...
    /**
     * VONNDA-MODIFICATION:
     * Modified to add the ability to set a custom callback. We wanted to submit via ajax instead of actually submitting the form
     */
    // Triggered from the My Saved Cards section
    saveCard: function(saveButton, callback)
    {
