    ...
    public function addFieldToFilter($field, $condition = null)
    {
        if ($field === 'order_items' && !$this->getFlag('product_filter_added')) {
            ...
            $this->addFieldToFilter([
                ...
                "{$this->getTable('sales_order_item')}.product_sku",
            ], [
                $condition,
                $condition,
            ]);
        ...
        if (count($items)) {
            ...
            $select = $connection->select()
                ->from([
                    ...
                    'product_skuss' => new \Zend_Db_Expr('GROUP_CONCAT(`sales_order_item`.product_sku SEPARATOR "|")'),
            ...
            foreach ($itemCollection as $item) {
                ...
                $productskuss = explode('|', $item['product_skuss']);
                $html = '';

                foreach ($productSkus as $index => $sku) {
                    $html .= sprintf('<div>%d x [%s] %s </div>', $productQtys[$index], $sku, $productNames[$index],$productskuss);
                }
