...
class Generic
{
    ...
    public function convertPaymentMethodToCard($paymentMethod)
    {
     ...
     public function convertPaymentMethodToCards($paymentMethod)
    {
        if (!$paymentMethod || empty($paymentMethod))
            return null;

        $card = json_decode(json_encode($paymentMethod));
        $card->id = $paymentMethod->id;
    ...
    public function listUpdateCards($customer, $params = array())
    {  
        try
        {
            $sources = $customer->sources;
            if (!empty($sources))
            {
                ...
                foreach ($data->autoPagingIterator() as $pm)
                {
                    $carddetails[] = $this->convertPaymentMethodToCards($pm);
                }
